...
#Scatterplot waarin het behaalde Eindcijfer wordt weergegeven ten opzichte van de absentie, met regressielijn. 

def custom_legend_name(new_names):
    for i, new_name in enumerate(new_names):
     fig13.data[i].name = new_name

fig13 = px.scatter(students, x='absences', y='G3', color='sex', trendline = 'ols',
                   title = "Eindcijfer (G3) ten opzichte van Absentie", width = 1000,
                  labels = {'age': 'Leeftijd', 'G3': 'Eindcijfer', 'sex': 'Geslacht', 'studytime':'Studeertijd'})
fig13.update_layout(yaxis_title="Cijfer", xaxis_title="Absenties")
custom_legend_name(['Vrouw','Man'])
